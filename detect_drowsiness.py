...
AUDIO_PATH = str(BASE_DIR) + '/alarm/alarm.wav'
PREDICTOR_PATH = str(BASE_DIR) + '/shape_predictor/shape_predictor_68_face_landmarks.dat'
FRAME_WIDTH = 480


def sound_alarm(audio):
	"""
	play an alarm sound
	"""
	playsound(audio)
...
# define two constants, one for the eye aspect ratio to indicate blink and then a second constant
	...
	for rect in rects:
		...
		if ear < EYE_AR_THRESH:
			...
			if COUNTER >= EYE_AR_CONSEC_FRAMES:
				...
				if (ALARM_ON):
					t = Thread(target=sound_alarm, args=(AUDIO_PATH,))
					t.deamon = True
					t.start()
				else:
					try:
